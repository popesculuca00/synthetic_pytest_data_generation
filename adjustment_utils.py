    ...
    for child in ast.walk(node):
        if isinstance(child, ast.Compare) and len(child.comparators) == 1:
            ...
            if (
                ...
                or actual_value_str.strip("()") == actual_str
                or eval_match
            ):
...
def replace_variable_with_literal(node, variable_name, new_value):
    ...
    for child in ast.walk(node):
        if isinstance(child, ast.Compare) and len(child.comparators) == 1:
            if (
                isinstance(child.comparators[0], ast.Name)
                and child.comparators[0].id == variable_name
            ):
...
def adjust_pytest(pytest, output):
    ...
    failure_report_strings[-1] = re.split(
        "---------- coverage", failure_report_strings[-1]
    )[0]
    ...
    for report in failure_report_strings:
        ...
        if "pytest.approx" in pytest_lines[line_number]:
            pytest_lines[line_number] = re.sub(
                "pytest\.approx\(([^,)]+),?[^)]*\)",
                lambda m: f"{ m.group(1) }",
                pytest_lines[line_number],
            )
            continue
        ...
        if error_type:
            ...
            if (error_type[1].lower() != "assertion") and (
                error_type[1].lower() != "synthax"
            ):
                ...
                pytest_lines[line_number] = (
                    leading_whitespace
                    + f"with pytest.raises({error_type[0]}):\n"
                    + leading_whitespace
                    + " " * 4
                    + pytest_lines[line_number]
                )
        ...
        if "assert false" in report.lower():
            pytest_lines[line_number] = re.sub(
                "(.*assert)(.*)",
                lambda m: f"{m.group(1)} not {m.group(2)}",
                pytest_lines[line_number],
            )
            continue
